...
# -*- coding: utf-8 -*-
"""
compose_cards_20260926.py
PIL 合成: 5 张商品图 + 详情文字 → 1080px 宽商品详情长图
然后 5 等份裁剪 + 1 张全图, 供 upload_cloudimgs.py 上传
(基于 compose_cards_20260925.py, 主题色调整为 9 月底秋分后深焙咖啡棕 + 燕麦奶霜白 + 深松针绿)
"""
import json
...
# 颜色 - 9 月底秋分后深焙咖啡棕 + 燕麦奶霜白 + 深松针绿 (国庆前换季/出游/润燥主题)
COLOR_BG = (252, 248, 240)  # 燕麦奶霜白底
COLOR_TEXT = (40, 32, 26)  # 深焙咖啡棕文字
COLOR_ACCENT = (74, 56, 42)  # 深焙咖啡棕主色
COLOR_ACCENT_2 = (96, 122, 86)  # 深松针绿强调
COLOR_TAG = (235, 224, 208)
COLOR_TAG_TEXT = (78, 60, 46)
COLOR_PRICE = (180, 56, 38)
COLOR_BORDER = (218, 206, 188)

# 卡片尺寸
CARD_H = 1620  # 每张卡片纵向 1620px
...
def render_card(idx, p):
    ...
    # 分类标签 (左)
    draw.rounded_rectangle([48, content_y + 36, 48 + 280, content_y + 86], radius=24, fill=COLOR_TAG)
    draw.text((68, content_y + 44), truncate(draw, p['category'], font_bold(26), 260), font=font_bold(26), fill=COLOR_TAG_TEXT)

    # 编号 (右)
    draw.text((WIDTH - 200, content_y + 38), f"NO.{idx:02d}", font=font_bold(46), fill=COLOR_ACCENT)

    # 商品名称 (大字)
    name = truncate(draw, p['name'], font_bold(38), WIDTH - 96)
    draw.text((48, content_y + 110), name, font=font_bold(38), fill=COLOR_TEXT)

    # 价格 + 价格说明
    draw.text((48, content_y + 170), p['price'], font=font_bold(38), fill=COLOR_PRICE)
    if p.get('price_note'):
        pn = truncate(draw, p['price_note'], font(20), WIDTH - 96)
        draw.text((48, content_y + 217), pn, font=font(20), fill=(80, 100, 120))

    # 分隔线
    draw.line([48, content_y + 265, WIDTH - 48, content_y + 265], fill=COLOR_BORDER, width=2)

    # 5 大卖点
    draw.text((48, content_y + 285), "✦ 5 大亮点", font=font_bold(28), fill=COLOR_ACCENT_2)
    for i, h in enumerate(p.get('highlights', [])[:5], 1):
        text = truncate(draw, f"{i}. {h}", font(22), WIDTH - 96)
        draw.text((56, content_y + 327 + (i - 1) * 48), text, font=font(22), fill=COLOR_TEXT)

    # 适合人群 + 颜色
    bot_y = content_y + 327 + 5 * 48 + 12
    draw.text((48, bot_y), f"🎯 适合: {p.get('suitable', '')}", font=font(20), fill=(80, 90, 110))
    bot_y += 30
    draw.text((48, bot_y), f"🎨 规格: {p.get('color', '')}", font=font(20), fill=(80, 90, 110))
...
def main():
    ...
    total_h = len(products) * CARD_H + 100
    ...
    full_draw.text((48, 65), data['title'], font=font(20), fill=(245, 235, 220))
    ...
    # 5 等份裁剪 (兼容 upload_cloudimgs.py 需要的 product_slice_1.jpg 命名)
    for idx in range(1, len(products) + 1):
        y = 100 + (idx - 1) * CARD_H
        crop = full.crop((0, y, WIDTH, y + CARD_H))
        slice_path = OUT_DIR / f'product_slice_{idx}.jpg'
        crop.save(slice_path, 'JPEG', quality=92)
        print(f"  ✅ 切片 {idx:02d}: {slice_path} ({(slice_path.stat().st_size / 1024):.0f}KB, {WIDTH}×{CARD_H})")
        # 同步保留 product_slice_01.jpg 命名
        slice_path_legacy = OUT_DIR / f'product_slice_{idx:02d}.jpg'
        crop.save(slice_path_legacy, 'JPEG', quality=92)

    # 复制为 product_card_full.jpg (upload_cloudimgs.py 需要)
    import shutil
    shutil.copy(full_path, OUT_DIR / 'product_card_full.jpg')
